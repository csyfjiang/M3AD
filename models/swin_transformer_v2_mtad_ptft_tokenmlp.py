# --------------------------------------------------------
# Swin Transformer V2 with AlzheimerMMoE and SimMIM - With ShiftedBlock
# Copyright (c) 2022 Microsoft
# Licensed under The MIT License [see LICENSE for details]
# Written by Ze Liu
# Modified for Alzheimer's Disease MMoE with SimMIM and ShiftedBlock
# --------------------------------------------------------
...
class ShiftedBlockLayer(nn.Module):
    ...
    def __init__(self, blocks, downsample=None):
        ...
        self.is_shifted_layer = True  # 添加标识
...
class SwinTransformerV2_AlzheimerMMoE_ShiftedBlock(nn.Module):
    """
    扩展版Swin Transformer V2 with Alzheimer MMoE and SimMIM
    集成临床先验信息和SimMIM预训练
    最后一个stage可选使用ShiftedBlock
    """
    ...
    def forward_features(self, x, clinical_prior=None, lbls_diagnosis=None, lbls_change=None, mask=None):
        ...
        # Original dual-task forward - 统一处理
        x_diagnosis, x_change = x, x
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("Testing SwinTransformerV2_AlzheimerMMoE with ShiftedBlock")
    print("4 Experts, 3 Diagnosis Classes, 3 Change Classes")
    print("=" * 80)
    ...
    for config in configs:
        ...
        test_change_labels = torch.randint(1, 4, (batch_size,)).to(device)  # 1-3
        ...
        with torch.no_grad():
            diag_out, change_out = model(
                ...
                lbls_diagnosis=test_diag_labels,  # 1-indexed labels
                lbls_change=test_change_labels
            )

        print(f"  ✓ Diagnosis output: {diag_out.shape} (expected: [{batch_size}, 3])")
        print(f"  ✓ Change output: {change_out.shape} (expected: [{batch_size}, 3])")
        ...
        for i, layer in enumerate(model.layers):
            if hasattr(layer, 'is_shifted_layer') and layer.is_shifted_layer:
        ...
        if config["use_shifted_last_layer"]:
            print("\n5. Testing different fusion types with ShiftedBlock:")
            fusion_types = ['adaptive', 'concat', 'add', 'hadamard']

            for fusion_type in fusion_types:
                try:
                    model_fusion = SwinTransformerV2_AlzheimerMMoE_ShiftedBlock(
                        img_size=img_size,
                        patch_size=patch_size,
                        in_chans=in_chans,
                        ...
                        fusion_stage=2,
                        fusion_type=fusion_type,
                        use_shifted_last_layer=True,
                        shift_mlp_ratio=1.0
                    ).to(device)

                    with torch.no_grad():
                        diag_out, change_out = model_fusion(
                            test_image,
                            clinical_prior=test_prior,
                            lbls_diagnosis=test_diag_labels,
                            lbls_change=test_change_labels
                    ...
                    print(f"  ✗ Fusion type '{fusion_type}' failed: {str(e)}")

            # 额外的详细检查
            print("\n6. Last layer detailed check:")
            last_layer = model.layers[-1]
            if hasattr(last_layer, 'is_shifted_layer') and last_layer.is_shifted_layer:
        ...
        # 测试专家利用率功能
        print("\n7. Testing expert utilization tracking:")
        try:
            expert_weights = model.get_expert_utilization(
                test_image[:1],  # 使用单个样本
                clinical_prior=test_prior[:1],
                lbls_diagnosis=test_diag_labels[:1],
                lbls_change=test_change_labels[:1]
            )
            print(f"  ✓ Expert utilization collected: {len(expert_weights)} layers with MMoE")

            # 显示第一个block的专家权重分布
            if expert_weights:
                first_weights = expert_weights[0]
                diag_weights = first_weights['diagnosis_weights']
                change_weights = first_weights['change_weights']
                print(f"  ✓ First block diagnosis weights shape: {diag_weights.shape}")
                print(f"  ✓ First block change weights shape: {change_weights.shape}")
        except Exception as e:
            print(f"  ✗ Expert utilization failed: {str(e)}")

        # 测试FLOPs计算
        print("\n8. Testing FLOPs calculation:")
        try:
            flops = model.flops()
            print(f"  ✓ FLOPs: {flops:,}")
    ...
    # 创建两个模型进行对比 - 需要提供完整参数避免window_size问题
    model_standard = SwinTransformerV2_AlzheimerMMoE_ShiftedBlock(
        img_size=img_size,
        patch_size=patch_size,
        in_chans=in_chans,
        num_classes_diagnosis=3,
        ...
        depths=[2, 2, 6, 2],
        num_heads=[3, 6, 12, 24],
        window_size=16,  # 明确指定window_size避免默认值7
        mlp_ratio=4.,
        qkv_bias=True,
        drop_rate=0.0,
        attn_drop_rate=0.0,
    ...
    model_shifted = SwinTransformerV2_AlzheimerMMoE_ShiftedBlock(
        img_size=img_size,
        patch_size=patch_size,
        in_chans=in_chans,
        num_classes_diagnosis=3,
    ...
    params_shifted = sum(p.numel() for p in model_shifted.parameters())

    print(f"Standard (All BasicLayerMMoE): {params_standard:,} parameters")
    print(f"With ShiftedBlock last layer: {params_shifted:,} parameters")
    print(
        f"Parameter reduction: {params_standard - params_shifted:,} ({(1 - params_shifted / params_standard) * 100:.1f}%)")

    # 测试不同融合阶段
    print("\n" + "=" * 60)
    print("Testing Different Fusion Stages:")
    print("=" * 60)

    fusion_stages = [0, 1, 2, 3]
    for stage in fusion_stages:
        try:
            model_stage = SwinTransformerV2_AlzheimerMMoE_ShiftedBlock(
                img_size=img_size,
                patch_size=patch_size,
                in_chans=in_chans,
                num_classes_diagnosis=3,
                ...
                depths=[2, 2, 6, 2],
                num_heads=[3, 6, 12, 24],
                window_size=16,  # 明确指定window_size
                mlp_ratio=4.,
                qkv_bias=True,
                drop_rate=0.0,
                attn_drop_rate=0.0,
            ...
            with torch.no_grad():
                diag_out, change_out = model_stage(
                    test_image[:1],
                    clinical_prior=test_prior[:1],
                    lbls_diagnosis=test_diag_labels[:1],
                    lbls_change=test_change_labels[:1]
            ...
            print(f"  ✓ Fusion at stage {stage} works correctly")
        except Exception as e:
            print(f"  ✗ Fusion at stage {stage} failed: {str(e)}")

    # 测试预训练与微调模式切换
    print("\n" + "=" * 60)
    print("Testing Pretrain/Finetune Mode Switch:")
    print("=" * 60)

    model_mode_test = SwinTransformerV2_AlzheimerMMoE_ShiftedBlock(
        img_size=img_size,
        patch_size=patch_size,
        in_chans=in_chans,
        num_classes_diagnosis=3,
    ...
    print(f"Initial mode - is_pretrain: {model_mode_test.is_pretrain}")

    # 切换到微调模式
    model_mode_test.set_pretrain_mode(False)
    print(f"After switching - is_pretrain: {model_mode_test.is_pretrain}")

    # 测试在微调模式下的前向传播
    try:
        with torch.no_grad():
            diag_out, change_out = model_mode_test(
                test_image[:1],
                clinical_prior=test_prior[:1]
            )
        print("  ✓ Forward pass in finetune mode successful")
    except Exception as e:
        print(f"  ✗ Forward pass in finetune mode failed: {str(e)}")
